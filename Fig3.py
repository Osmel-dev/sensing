# This script reproduces the results of Fig. 3 in the manuscript 
...
# =========================================
# Precision versus number of devices
# =========================================
...
avgPrecisionVSDevices = np.zeros((len(DList), nAPUs-1))

precisionPerSeed = np.zeros((len(DList), MCIter))
zScene = radarCrossSection.reshape(I,I)

for i in np.arange(0,nAPUs-1):
    for DIdx, D in enumerate(DList):
        for seed in np.arange(MCIter):
            sceneToFuse = results[(S[i],C[i])]["reconstScene"][DIdx,seed,:,:,:]
            primalRes = results[(S[i],C[i])]["primalResidual"][DIdx,seed,:]

            # fusing the images 
            zFused = utils.fusion(sceneToFuse,primalRes) 

            # compute precision
            avgPrecisionVSDevices[DIdx,i] += (1/MCIter)*utils.estQuality(zFused,zScene)

fig1, ax1 = plt.subplots(num=1, figsize=(8, 6), clear=True)

ax1.plot(DList, avgPrecisionVSDevices[:,0], linestyle="-", marker="o", markersize=6,
         color="black", linewidth=1.5, label=r"$C=1,\ S=7$")
ax1.plot(DList, avgPrecisionVSDevices[:,1], linestyle="-", marker="+", markersize=6,
         color="#D95319", linewidth=1.5, label=r"$C=2,\ S=6$")
ax1.plot(DList, avgPrecisionVSDevices[:,2], linestyle="-", marker="*", markersize=6,
         color="#EDB120", linewidth=1.5, label=r"$C=3,\ S=5$")
ax1.plot(DList, avgPrecisionVSDevices[:,3], linestyle="-", marker="x", markersize=6,
         color="#7E2F8E", linewidth=1.5, label=r"$C=4,\ S=4$")
ax1.plot(DList, avgPrecisionVSDevices[:,4], linestyle="-", marker="^", markersize=6,
         color="#77AC30", linewidth=1.5, label=r"$C=5,\ S=3$")
ax1.plot(DList, avgPrecisionVSDevices[:,5], linestyle="-", marker="s", markersize=6,
         color="#A2142F", linewidth=1.5, label=r"$C=6,\ S=2$")
ax1.plot(DList, avgPrecisionVSDevices[:,6], linestyle="-", marker="d", markersize=6,
         color="#0072BD", linewidth=1.5, label=r"$C=7,\ S=1$")

ax1.grid(True)
ax1.set_axisbelow(True)
ax1.set_xticks(DList)
ax1.set_xlim(DList.min() + 2, DList.max())
ax1.set_xlabel(r"$D$")
